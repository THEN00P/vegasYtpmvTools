...
namespace Monad.FLParser
{
    internal class ProjectParser
    {
        ...
        static Encoding _enc16 = Encoding.Unicode;
        ...
        private void ParseTextEvent(Enums.Event eventId, BinaryReader reader)
        {
            ...
            // check if it's a UTF16 string
            // dirty but works
            for (int i = 1; i < dataLen / 2; i++)
            {
                if (dataBytes[i] == dataBytes[i + 2] && dataBytes[i] == 0)
                {
                    isUTF16 = true;
                    break;
                }
            ...
            if (isUTF16)
                stringDecoder = _enc16.GetDecoder();
            else
                stringDecoder = _enc8.GetDecoder(); 

            int nChars = stringDecoder.GetCharCount(dataBytes, 0, dataLen);
            char[] chars = new char[nChars];

            nChars = stringDecoder.GetChars(dataBytes, 0, dataLen, chars, 0);
            ...
            //LogWriter.LogWrite($"text: {unicodeString}");
